...
FILE_DICTIONARY = {
    "ycbv_zebra_segm": (
        "zebraposesat-effnetb4_ycbv-test_5ed0eecc-96f8-498b-9438-d586d4d92528",
        "segm",
        False,
    ),  # https://bop.felk.cvut.cz/sub_info/3131/
    "ycbv_gdrnppdet_bbox": (
        "gdrnppdet-pbrreal_ycbv-test_abe6c5f1-cb26-4bbd-addc-bb76dd722a96",
        "bbox",
        True,
    ),  # https://bop.felk.cvut.cz/sub_info/2743/
}
...
if len(p["gt_from_datasets"]) > 0:
    ...
    FILE_DICTIONARY = {f"{ds}_gt": (f"gt-results_{ds}-test_coco", "bbox", False) for ds in p["gt_from_datasets"]}
    EXPECTED_OUTPUT = {
        f"{ds}_gt": {
            "AP": 1.0,
            "AP50": 1.0,
...
for dataset_method_name, (sub_name, ann_type, compressed) in tqdm(FILE_DICTIONARY.items(), desc="Executing..."):
        ...
        for key, expected_score in EXPECTED_OUTPUT[sub_short_name].items():
            ...
            if eval_score is not None:
                if abs(eval_score - expected_score) < args.tolerance:
                    ...
                    misc.log(f"{sub_short_name}: {key} - FAILED. Expected: {expected_score}, Actual: {eval_score}")
            else:
